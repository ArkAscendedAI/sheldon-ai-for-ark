...
### Other assets in the current mod

The three Blueprints above are the core. The shipping mod has grown well beyond this overview, and the Blueprint source in [`mod/`](mod/) is the source of truth. The current mod also includes:

- **BP_SheldonEventBuff, BP_SheldonDinoEventBuff, BP_SheldonInputBuff** - PrimalBuff-based components that emit proactive game events to the bridge (player death and damage, dino tame and death) and carry the per-player Sheldon status icon.
- **WBP_ChatRow** - a pre-designed chat-row widget instantiated per message (cook-safe per-message styling), used by WBP_SheldonChat.
- **WBP_SheldonSubtitle** - an on-screen subtitle overlay.
- **DT_SheldonChatStyles, DT_RichProbe** - data tables backing the chat styling.
- **Sheldon_buff_icon** - the status icon texture for the Sheldon buff.
- **PrimalGameData_BP_SheldonAI, ModDataAsset_SheldonAI** - the mod's PrimalGameData and mod-data-asset registration.

The live getter set has likewise grown to 25+ per-survivor and world queries, plus an admin command path. See the bridge's tool registry and the Blueprint source for the current list.
...
- "On WebSocket Message Recieved" is misspelled in the DevKit, so use that exact spelling
- Map Extensions, NOT GameMode (only first mod's GameMode loads)
- CurseForge cloud cooking can target PC and, later, console; SheldonAI is built and supported on PC for now
- The bridge defaults to ws://. A future console build would need wss:// (console clients validate certificates); PC clients accept ws:// or a self-signed wss://
- Create WebSocket returns the last valid ID on failure (a known DevKit quirk; verify the result before using it)
...
3. Choose PC-Only (auto-approve). Console / cross-platform is not supported yet, so PC-Only is the right choice.
