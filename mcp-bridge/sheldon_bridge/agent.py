...
class AgentResult:
    ...
    duration_ms: float
    error: str | None = None
    transient: bool = False  # canned non-error rejection (max-iter / rate-limit) — show the UX text
    #                          but do NOT persist it as assistant history (LEAK-02). error stays None.
...
class Agent:
    ...
    async def run(
        ...
        Args:
            ...
            extra_context: Per-turn values merged into the tool ctx (e.g. the
                campaign store + requesting survivor's eos_id for memory tools).
        ...
        # NOTE: the per-player message rate-limit ("requests") is now enforced at ADMISSION in
        # server._handle_player_message — BEFORE the expensive context assemble + a turn-task slot —
        # so a spammer is rejected cheaply and can't shed other players via the admission cap
        # (SERVER-11). Removed from here to avoid double-counting the sliding window.
        ...
        return AgentResult(
            ...
            total_cost=total_cost,
            duration_ms=(time.time() - start_time) * 1000,
            transient=True,  # canned fallback — don't persist as assistant history (LEAK-02)
        )
